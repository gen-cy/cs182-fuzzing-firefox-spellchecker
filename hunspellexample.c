...
int main(int argc, char** argv) {
  ...
  outfile.open("input_list.txt", std::ios_base::app);
  ...
  /* open the words to check list */
   std::ifstream wtclst(argv[1], std::ios_base::in);
  if (!wtclst.is_open()) {
    fprintf(stderr, "Error - could not open file of words to check\n");
    exit(1);
  }

  Hunspell* pMS = new Hunspell("en_US.aff", "en_US.dic");
